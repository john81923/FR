...
def onet_preprocess_(image, rectangle, input_shape, gray_scale=True):
    ...
    scale_img = image.crop([cx-size//2, cy-size//2, cx+size//2, cy+size//2]).resize(input_shape, Image.BILINEAR)
    if gray_scale:
    ...
    return x, 1.0*size / input_shape[0], 1.0*size / input_shape[1]

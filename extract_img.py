...
def pre_process(image_path):
    ...
    ret = None
    #test_path = "/home/craig/kneron_development/kneron_tw_models/framework/face_recognition/kfr/00124_0001.bmp"
    if len(dets)>0:
...
if __name__ == "__main__":
    root = '/mnt/sdc/craig/Guizhou_NIR_dataset/'
    target = '/mnt/sdc/craig/Guizhou_NIR_dataset_kfr_0304/'
    create_folder(target)
    indoor_path = os.path.join(root,'indoor_testdata')
    outdoor_path = os.path.join(root,'outdoor_testdata')
    indoor_folder = os.listdir(indoor_path)
    outdoor_folder = os.listdir(outdoor_path)

    for folder in indoor_folder:
        src_folder = os.path.join(indoor_path,folder)
        tgt_folder = os.path.join(target,folder)
        create_folder(tgt_folder)
        image_set = os.listdir(src_folder)
        for image in image_set:
            tgt_image_path = os.path.join(tgt_folder,"indoor_"+image)[:-4] +'.png'
            if os.path.isfile(tgt_image_path):
                ...
                if items>0:
                    ...
                    #output_image.save(tgt_image_path)
    '''
    for folder in outdoor_folder:
        src_folder = os.path.join(outdoor_path,folder)
        tgt_folder = os.path.join(target,folder)
        create_folder(tgt_folder)
        image_set = os.listdir(src_folder)
        for image in image_set:
            tgt_image_path = os.path.join(tgt_folder,"outdoor_"+image)[:-4] +'.png'
            if os.path.isfile(tgt_image_path):
                pass
            else:
                src_image_path  = os.path.join(src_folder,image)

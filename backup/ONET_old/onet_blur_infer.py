...
class onet_detector:
    def __init__(self, model_path, input_shape=(48, 48)):
        self.model = keras.models.load_model(model_path)
        self.input_shape = input_shape
        # self.score_thres = score_thres
        return

    def run(self, image, rectangle=None):
        """
        image: str or np.ndarray or ImageObject
        rectangle: list, [x, y, w, h, score]

        -return: landmarks, list of 12 floats or []
        """
        ...
        img_data, scale_w, scale_h = onet_preprocess_(image, rectangle, self.input_shape, gray_scale=False)
    ...
    def run_batch(self, image, rectangle_list):
        ...
        for rectangle in rectangle_list:
            img_data, scale_w, scale_h = onet_preprocess_(image, rectangle, self.input_shape)
        ...
        for i in range(len(rectangle_list)):
            landmarks = onet_postprocess_([outputs[0][i], outputs[1][i]], scale_batch[i][0], scale_batch[i][1],
                                          self.input_shape, rectangle_list[i])
...
if __name__ == "__main__":
        ...
        if landmarks_list:
            for i in range(len(landmarks_list)):
                for j in range(0, 10, 2):
                    ...
                    ax.add_patch(circle)
                ax.text(landmarks_list[i][8], landmarks_list[i][9], "%.2f" % landmarks_list[i][-1],
    ...
    detector = onet_detector("/home/bodong/playground/layer/rgb2gray_Laplacian_onet.hdf5")

    image = np.random.rand(48,48,3)
    landmarks = detector.run(image)
    print(landmarks)

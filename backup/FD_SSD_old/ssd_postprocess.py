...
def postprocess_(outputs, anchor, scale, input_shape, w_ori, h_ori, score_thres, only_max, iou_thres, batch_idx=0):
    """
    decode one sample, given batch_idx, among SSD output batch to rectangle. 
    outputs: list(np.ndarry), 4 X [batch, h, w, anchors*num_class] + 4 X [batch, h, w, anchors*4]
    anchor: dict(str:np.ndarray), 4 X [h, w, anchors, 4].
    scale: float
    ...
    batch_idx: int, indicating which sample to do postprocessing among this batch
    ...
    for stage in range(nstages):
        ...
        logits = outputs[stage][batch_idx,...]
        regr = outputs[stage+nstages][batch_idx,...]
            ...
            for j in range(ncols):
                for k in range(nanchors):
                    ...
                    if score > score_thres:
                        ...
                        dets.append([cx-w//2, cy-h//2, w, h, score])
    # do nms
    ...
    if len(dets) > 0:
        ...
        dets[:,:2] = np.maximum(dets[:,:2], 0)
        dets[:,2:4] = np.minimum(dets[:,2:4], np.array([[w_ori, h_ori]]))
